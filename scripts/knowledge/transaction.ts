...
function isTargetPath(path: string): boolean { return path.startsWith('src/content/') || path.startsWith('src/data/routes/') || path.startsWith('public/media/') }
...
export async function applyBatchTransaction(root: string, batch: ScannedBatch, options: ApplyTransactionOptions = {}): Promise<void> {
  ...
  await assertOperationPreconditions(workspace, batch)
  ...
  const paths = [...new Set(batch.manifest.operations.flatMap((operation) => operation.move_from ? [operation.path, operation.move_from] : [operation.path]))]
