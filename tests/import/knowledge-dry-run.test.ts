...
describe('knowledge update dry-run', () => {
  ...
  it('simulates an ordered media batch without changing the source workspace', async () => {
    ...
    const manifest = Buffer.from(JSON.stringify({ schema_version: 1, batch_id: 'batch-20260802-001-test', created_at: '2026-08-02T00:00:00.000Z', released_on: '2026-08-02', base_content_version: '2026.08.01-01', target_content_version: '2026.08.02-01', summary: 'test media import', operations: [{ operation_id: 'op-0001', action: 'add', kind: 'media', path: 'public/media/test.png', entity_id: 'test-media', payload_sha256: hash(payload) }] }))
    await writeFile(resolve(root, 'inbox/batches/batch-20260802-001-test.zip'), storedZip([{ name: 'batch.json', data: manifest }, { name: 'payload/public/media/test.png', data: payload }]))
    const report = await dryRunKnowledgeUpdate({ repositoryRoot: root, runId: 'test-run', git: { head: 'test', clean: true } })
    expect(report).toMatchObject({ target_content_version: '2026.08.02-01', ordered_batch_ids: ['batch-20260802-001-test'], conclusion: 'dry-run 未修改正式源' })
    expect(await readFile(resolve(root, 'inbox/reports/test-run-dry-run.json'), 'utf8')).toContain('APPLY:1:2026.08.02-01')
