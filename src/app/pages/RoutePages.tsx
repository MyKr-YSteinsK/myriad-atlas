...
export function RoutesPage() {
  ...
  return <section className="atlas-page route-list-page"><PageHeader kicker="PATHS" title="路线" />
    ...
    <ol className="route-index">{routeRecords.map((record) => {
      ...
      const currentIndex = target ? units.findIndex((unit) => unit.node_id === target.unit.node_id) : undefined
      return <li key={route.id} data-completed={progress.ratio === 1}><span className="route-code">{route.code}</span><div className="route-entry"><header><h2><Link to={'/route/' + route.id}>{route.name}</Link></h2><output aria-label={route.name + ' 完成度'}>{Math.round(progress.ratio * 100)}%</output></header><p className="route-summary">{route.summary}</p><MiniRoute units={units.map((unit) => ({ role: visualRouteRole(unit.role) }))} currentIndex={currentIndex} completed={progress.completed} label={route.name + ' 路线进度'} /><div className="route-position"><span>CURRENT</span><strong>{routeUnitNumber(currentUnit)}</strong><span>NEXT</span><strong>{routeUnitNumber(target?.unit)}</strong></div><footer><span>{progress.completed} / {progress.total}</span><Link to={target ? '/node/' + target.unit.node_id + '?source=route&route=' + route.id + '&stage=' + target.stageId + '&module=' + target.moduleId : '/route/' + route.id}>{progress.ratio === 1 ? '查看总结' : position ? '继续' : '开始'}</Link></footer></div></li>
    })}</ol>
  </section>
}
...
export function RouteDetailPage() {
  ...
  return <section className="atlas-page route-detail-page"><PageHeader variant="context" kicker={route.code + ' · PATH'} title={route.name} summary={route.summary} meta={<strong>{Math.round(progress.ratio * 100)}%</strong>} actions={target
    ...
    : <span>完成</span>} />
    <section className="route-detail-overview" aria-label="路线结构概览"><MiniRoute units={units.map((unit) => ({ role: visualRouteRole(unit.role) }))} currentIndex={currentIndex} completed={progress.completed} label={route.name + ' 路线总进度'} /><MetricBar value={progress.completed} max={progress.total} label="主线与综合任务" tone="success" /><p>可选节点不计入 100% 路线进度。</p></section>
    <ol className="route-stages">{route.stages.map((stage, stageIndex) => <li key={stage.id}><header className="route-stage-header"><p className="atlas-coordinate">STAGE {String(stageIndex + 1).padStart(2, '0')}</p><h2>{stage.name}</h2><p>{stage.summary}</p></header>
      {stage.modules.map((module, moduleIndex) => <section key={module.id} className="route-module"><header><span>MODULE {String(moduleIndex + 1).padStart(2, '0')}</span><div><h3>{module.name}</h3><p>{module.summary}</p></div></header><ol>{[...module.units].sort((a, b) => a.order - b.order).map((unit) => {
        ...
        const glyph = done ? 'completed' : current ? 'current' : 'unread'
