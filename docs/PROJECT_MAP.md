...
- `src/app/components/visual/`：可复用的 Atlas 图形信息语法（状态图形、阅读与路线轨道、比例条、指标带和确定性迷你航图）；`src/app/components/` 与 `src/app/styles/global.css`：页面标题、链接与语义视觉 token，以及 Light / Dark / Warm 主题、safe area、响应式探索布局、阅读排版、焦点样式和减少动态效果。
